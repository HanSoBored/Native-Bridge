    ...
    for (int i = 1; i < r; i++) {
        if (json_eq(json, &t[i], key) == 0) {
            if (i + 1 < r) {
                ...
                if (len >= (int)max) len = max - 1;
                memcpy(out, json + t[i + 1].start, len);
                out[len] = '\0';
                return 1;
            }
        }
    ...
    while (1) {
        ...
        if (res_hdr.type == RESP_STREAM_CHUNK || res_hdr.type == RESP_STREAM_ERR || res_hdr.type == RESP_SUCCESS || res_hdr.type == RESP_ERROR) {
            ...
            if (current_len + needed < max_out - 1) {
                ...
                // Buffer full, stop appending but continue flush loop
            }
        ...
        if (strcmp(method, "initialize") == 0) {
            ...
            if (strcmp(tool_name, "device_ping") == 0) {
                ...
                if (is_command_blocked(first_token)) {
                    ...
                    exec_res = run_bridge_command(CMD_EXEC, cmd_str, strlen(cmd_str) + 1, raw_output, sizeof(raw_output));
                }
            }
